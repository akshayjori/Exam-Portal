...
public class UserService {
	...
	public boolean registerNewUser(User user) {
		...
		try {
			...
			String myQuery = "INSERT INTO user(firstname, lastname, dateofbirth, gender, contactNo, email, username, password)"
							+" values(?, ?, ?, ?, ?, ?, ?, ?);";
			...
			myStatement.setString(6, user.getEmail());
			myStatement.setString(7, user.getUsername());
			myStatement.setString(8, user.getPassword());
	...
	public User checkForExistingUser(User user) {
		...
		try {
			...
			String myQuery = "SELECT userId, firstname, username, password FROM user WHERE username= ? AND password= ?;";
			myStatement = connection.prepareStatement(myQuery);
